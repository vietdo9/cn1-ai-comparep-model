"""Evaluation script on test set with TensorBoard visualization logging."""
import argparse
import json
...
def parse_args():
    parser = argparse.ArgumentParser(description='Evaluate EfficientNet AI Detector')
    parser.add_argument('--checkpoint', type=str, required=True, help='Path to checkpoint .pt')
    parser.add_argument('--config', type=str, default=None, help='Path to config yaml (auto-detect if None)')
...
def main():
    ...
    report_path = save_dir / 'test_report.json'
    ...
    writer.close()
    print(f"Visualizations logged to: {tb_dir}")
